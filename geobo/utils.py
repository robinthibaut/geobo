...
def readcsv_drill(fname, prop_names, pos_names, cartesian=True):
    ...
    if cartesian:
        xdrill, ydrill, zdrill = (
            drill[pos_names[0]].values,
            drill[pos_names[1]].values,
            drill[pos_names[2]].values,
        )
    else:
        xdrill, ydrill, zdrill = spherical2cartes(
            drill.East.values,
            drill.North.values,
            drill.Elev.values,
            drill.Azimuth.values * np.pi / 180.0,
            (90 - drill.Dip.values) * np.pi / 180.0,
            0.5 * (drill.DepthFrom.values + drill.DepthTo.values),
        )
    data = pd.DataFrame()
    data["x"] = xdrill
    data["y"] = ydrill
...
def readraster_survey(fname, pixres=None, clipext=None):
    ...
    if pixres is not None:
        assert (
            ypixres == xpixres
        ), " Raster pixelsize (resolution) is not the same in vertical and horizontal direction"
        array = zoom(array, ypixsize / pixres)
        xpixres = ypixres = pixres
    # Define pixel coordinates of original image
    xx, yy = np.meshgrid(
        np.linspace(
            bounds[0] + 0.5 * xpixres, bounds[2] - 0.5 * xpixres, array.shape[1]
        ),
        np.linspace(
            bounds[1] + 0.5 * ypixres, bounds[3] - 0.5 * ypixres, array.shape[0]
        ),
    )
    if clipext is not None:
        ...
        if (
            ...
            # Clip array:
            array = array[
                ((xx - 0.5 * xpixres) >= clipext[0])
                & ((xx + 0.5 * xpixres) < clipext[2])
                & ((yy - 0.5 * ypixres) >= clipext[1])
                & ((yy + 0.5 * ypixres) >= clipext[3])
            ]
            xx2 = xx[
                ((xx - 0.5 * xpixres) >= clipext[0])
                & ((xx + 0.5 * xpixres) < clipext[2])
            ...
            array = array.reshape(
                np.round((yy2.max() - yy2.min()) / ypixres + 1).astype(int),
                np.round((xx2.max() - xx.min()) / xpixres + 1).astype(int),
            )
            xx2 = xx2.reshape(
                np.round((yy2.max() - yy2.min()) / ypixres + 1).astype(int),
                np.round((xx2.max() - xx.min()) / xpixres + 1).astype(int),
            ...
            print(
                "WARNING: Clip extent exceeds image boundary!... No clipping is applied"
            )
    else:
        xx2, yy2 = xx, yy
