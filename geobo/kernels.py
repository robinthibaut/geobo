...
def calcDistanceMatrix(
    nDimPoints,
    distFunc=lambda deltaPoint: np.sum(
        deltaPoint[d] ** 2 for d in range(len(deltaPoint))
    ),
):
...
def gpkernel_sparse(D2, gamma):
    ...
    res[D2 < gamma] = (2 + np.cos(2 * np.pi * D2[D2 < gamma] / gamma)) / 3.0 * (
        1 - D2[D2 < gamma] / gamma
    ) + 1 / (2.0 * np.pi) * np.sin(2 * np.pi * D2[D2 < gamma] / gamma)
    ...
    res[D2 <= abs(l2 - l1) / 2.0] = (
        ...
        * (
            ...
            * np.sin(
                ...
                * lmin
                / lmax
                * np.cos(2 * np.pi * D2[D2 <= abs(l2 - l1) / 2.0] / lmax)
            )
        )
    )
    # if (D2 >= abs(l2 - l1)/2.) & (D2<=(l1 + l2)/2.):
    res[(D2 >= abs(l2 - l1) / 2.0) & (D2 <= (l1 + l2) / 2.0)] = (
        2.0
        / (3 * np.sqrt(l1 * l2))
        * (
            lmean
...
def create_cov(D2, gplength, crossweights=[1, 1, 1], fkernel="sparse"):
    ...
    if fkernel == "matern32":
        kcov1 = np.vstack(
            [
                gpkernel_matern32(D2, params[0]),
                w3 * gpkernel_matern32_2(D2, params[[0, 1]]),
                w1 * gpkernel_matern32_2(D2, params[[0, 2]]),
            ]
        )
        kcov2 = np.vstack(
            [
                w3 * gpkernel_matern32_2(D2, params[[1, 0]]),
                gpkernel_matern32(D2, params[1]),
                w2 * gpkernel_matern32_2(D2, params[[1, 2]]),
            ]
        )
        kcov3 = np.vstack(
            [
                w1 * gpkernel_matern32_2(D2, params[[2, 0]]),
                w2 * gpkernel_matern32_2(D2, params[[2, 1]]),
                gpkernel_matern32(D2, params[2]),
            ]
        )
    if fkernel == "sparse":
        kcov1 = np.vstack(
            [
                gpkernel_sparse(D2, params[0]),
                w3 * gpkernel_sparse2(D2, params[[0, 1]]),
                w1 * gpkernel_sparse2(D2, params[[0, 2]]),
            ]
        )
        kcov2 = np.vstack(
            [
                w3 * gpkernel_sparse2(D2, params[[1, 0]]),
                gpkernel_sparse(D2, params[1]),
                w2 * gpkernel_sparse2(D2, params[[1, 2]]),
            ]
        )
        kcov3 = np.vstack(
            [
                w1 * gpkernel_sparse2(D2, params[[2, 0]]),
                w2 * gpkernel_sparse2(D2, params[[2, 1]]),
                gpkernel_sparse(D2, params[2]),
            ]
        )
    if fkernel == "exp":
        kcov1 = np.vstack(
            [
                gpkernel(D2, params[0]),
                w3 * gpkernel2(D2, params[[0, 1]]),
                w1 * gpkernel2(D2, params[[0, 2]]),
            ]
        )
        kcov2 = np.vstack(
            [
                w3 * gpkernel2(D2, params[[1, 0]]),
                gpkernel(D2, params[1]),
                w2 * gpkernel2(D2, params[[1, 2]]),
            ]
        )
        kcov3 = np.vstack(
            [
                w1 * gpkernel2(D2, params[[2, 0]]),
                w2 * gpkernel2(D2, params[[2, 1]]),
                gpkernel(D2, params[2]),
            ]
        )
